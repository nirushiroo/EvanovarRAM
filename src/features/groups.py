...
def move_group(old_index: int, new_index: int) -> bool:
    """Move a group to a new final slot and persist the order.

    ``new_index`` is the index in the list *after* the item is removed
    (e.g. with [A, B, C], move_group(0, 1) yields [B, A, C]). Callers that
    think in "insert before tab T of the bar before the move" must apply the
    usual shift: subtract one when the target is past the old position.
    """
    data = _load()
    ...
    if new_index == old_index:
        return False
    name = groups.pop(old_index)
    groups.insert(new_index, name)
    _save(data)
    return True
