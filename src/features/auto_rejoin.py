...
def arm_accounts(
    ...
    """Start Auto-Rejoin workers for accounts that have a saved config.

    Returns (started, skipped). Accounts without a configuration are skipped
    rather than failing, which lets a launch profile arm whatever it can.
    """
    if on_status is None:
        on_status = lambda account, status: print( # noqa: E731
            f"[Auto-Rejoin] [{account}] {status}"
        )
    ...
    for account in accounts:
        config = configs.get(account)
        if not config:
            skipped.append(account)
            continue
        existing = workers.get(account)
        if existing is not None and existing.is_alive():
            started.append(account)
            continue
        worker = AutoRejoinWorker(account, config, manager, on_status=on_status)
...
def disarm_accounts(accounts: list[str], workers: dict) -> list[str]:
    """Stop Auto-Rejoin workers for the given accounts. Returns what stopped."""
    stopped: list[str] = []
    for account in accounts:
        worker = workers.pop(account, None)
