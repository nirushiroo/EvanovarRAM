...
def start_session(
    ...
    """Launch every account in a saved session.

    Unlike join_place_all this uses the session's own place, private server,
    job ID, launcher and delay rather than the global launch settings.
    """
    data = sessions_mod.normalize_session(session)
    if not data:
        result = OperationResult.failure(
            "SESSION_INVALID",
            "Session Is Invalid",
    ...
    private_server = str(data.get("private_server") or "")
    ...
    if not accounts:
        on_done(False, OperationResult.failure(
            ...
            "Add at least one account to this session before starting it.",
        ))
        return
    if not place_id and not private_server:
        on_done(False, OperationResult.failure(
            "SESSION_NO_DESTINATION",
            "Session Has No Destination",
            "Set a Place ID or a private server link before starting this session.",
        ))
        return

    settings = load_ui_settings()
    launcher = str(data.get("launcher") or "") or settings.get("roblox_launcher", "default")
    custom_path = str(data.get("custom_launcher_path") or "") or settings.get(
        "custom_roblox_launcher_path", ""
    )

    print(f"[INFO] start_session: '{name}' -> {len(accounts)} accounts, place {place_id or '-'}")
    ...
    def _worker():
        ...
        for index, username in enumerate(accounts):
            if username not in manager.accounts:
                failures.append((username, OperationResult.failure(
                    "ACCOUNT_NOT_FOUND",
                    "Account Not Found",
                    f"Account not found: {username}",
                )))
                continue
            try:
                result = ensure_result(manager.launch_roblox(
                ...
                if result:
                    ...
                    failures.append((username, result))
                print(f"[{'SUCCESS' if result else 'ERROR'}] start_session {username}: {'OK' if result else 'FAIL'}")
            except Exception as exc:
                print(f"[ERROR] start_session {username}: {exc}")
                failures.append((username, unexpected_result(f"Starting session for {username}", exc)))
            if index < len(accounts) - 1:
                _wait_between_launches(index, len(accounts), delay, accounts[index + 1])

        result = _batch_launch_result("Started", len(accounts), success, failures)
        print(f"[INFO] start_session '{name}' done: {result.message}")
        on_done(bool(result), result)

    threading.Thread(target=_worker, daemon=True, name=f"session-{name}").start()
...
def start_anti_afk_from_settings() -> bool:
    """Start Anti-AFK using the settings saved by the Anti-AFK controls."""
    settings = load_ui_settings()
    try:
        press_count = max(1, int(settings.get("anti_afk_press_count", 1) or 1))
        interval = max(1, int(settings.get("anti_afk_interval", 10) or 10))
    except (TypeError, ValueError):
        press_count, interval = 1, 10

    start_anti_afk(
        str(settings.get("anti_afk_key", "w") or "w"),
        press_count,
        interval,
        bool(settings.get("anti_afk_tooltip_enabled", True)),
    )
    return True
