...
from PySide6.QtCore import (
    QEvent, QObject, QPoint, QRect, QRectF, QSize, Qt, QTimer, QUrl, Signal,
)
from PySide6.QtGui import (
    QAction, QColor, QCursor, QFont, QIcon, QPainter, QPainterPath,
    QImage, QImageReader, QKeySequence, QMovie, QPalette, QPixmap, QPolygon, QRegion,
...
import features.extension_bridge as extension_bridge_mod
...
class _DragDropFilter(QObject):
    ...
    HOLD_LIST_MS = 800 # account list: still-hold also starts a drag
    HOLD_TABS_MS = 120 # group tabs: short hold so it still feels like a swipe
    DRAG_THRESHOLD_PX = 8 # hold + move this far starts the drag immediately
    ...
    def __init__(
        ...
        get_avatar=None,
        parent=None,
        row_labels: "Callable[[], list[str]] | None" = None,
        hold_ms: int | None = None,
    ):
        super().__init__(parent)
        self._list = list_widget
        self._get_avatar = get_avatar # Callable[[str], QPixmap|None] | None
        # Optional override for ordering targets whose items are plain widgets
        # (e.g. the group tab bar) rather than QListWidget rows.
        self._row_labels = row_labels
        self._hold_ms = hold_ms if hold_ms is not None else self.HOLD_LIST_MS
        ...
        # avoid calling viewport() on a deleted C++ object at teardown; plain
        # widgets (the group bar) ARE their own event target.
        self._viewport = (
            list_widget.viewport() if hasattr(list_widget, "viewport") else list_widget
        )
        # Widgets whose mouse events drive this filter. For the group tab bar
        # this includes every tab button: presses land on the child button and
        # never reach a filter installed only on the parent widget.
        self._targets: set = {self._viewport}

        self._indicator = QFrame(self._viewport)
        if self._row_labels is not None:
            self._indicator.setFixedWidth(2) # vertical line between tabs
        else:
            self._indicator.setFixedHeight(2) # horizontal line between rows
    ...
    def _on_press(self, event) -> bool:
        if event.button() != Qt.MouseButton.LeftButton:
            return False
        # Viewport-local coords: events may arrive from a child tab button,
        # whose local positions differ from the bar's.
        local = self._viewport.mapFromGlobal(event.globalPosition().toPoint())
        if self._row_labels is not None:
            row = self._row_at(local)
            label = ""
            labels = self._row_labels()
            if 0 <= row < len(labels):
                label = labels[row]
            if row < 0 or not label:
                return False
    ...
    def _row_at(self, local_pos) -> int:
        # Index of the child widget under viewport-local local_pos within the
        # shared layout, counting only the reorderable widgets (skips the
        # trailing "+" and stretch by construction: row_labels() lists just
        # those widgets).
        best = -1
        for index, widget in enumerate(self._ordered_widgets()):
            if widget is None:
                continue
            top_left = widget.mapTo(self._viewport, QPoint(0, 0))
    ...
    def _list_contains_viewport(self, point) -> bool:
        """True when a viewport-local point falls inside the visible bar.

        The horizontal scroll can clip tabs; hits outside the visible strip
        must not reorder anything.
        """
        rect = QRect(QPoint(0, 0), self._viewport.size())
    ...
    def _begin_drag(self):
        if self._dragging or self._press_row < 0:
            return
        self._dragging = True
    ...
    def _on_move(self, event) -> bool:
        ...
        if not self._dragging:
            if not self._hold_timer.isActive() or self._press_pos.isNull():
                return False
            # Hold-and-drag: moving past the threshold starts the drag right
            # away. Holding still for the full hold duration starts it too.
            if (gpos - self._press_pos).manhattanLength() <= self.DRAG_THRESHOLD_PX:
                return False
        ...
        local = self._viewport.mapFromGlobal(gpos)
        self._drop_row = self._compute_drop_row(local) # drop-before row
    ...
    def _compute_drop_row(self, local_pos) -> int:
        if self._row_labels is not None:
            ...
            for index, widget in enumerate(widgets):
                ...
                mid = rect.left() + rect.width() // 2 # tabs are horizontal
                if local_pos.x() < mid:
                    return index
            return count # past the last tab
    ...
    def _show_indicator(self, insert_before: int):
        if self._row_labels is not None:
            ...
            if count == 0:
                ...
                return
            if insert_before <= 0:
                rect = self._widget_rect(widgets[0])
                x = rect.left()
            elif insert_before >= count:
                rect = self._widget_rect(widgets[count - 1])
                x = rect.right()
            else:
                x = self._widget_rect(widgets[insert_before]).left()
            self._indicator.setGeometry(x - 1, 0, 2, self._viewport.height())
...
class _GroupTabDragFilter(_DragDropFilter):
    # Drag filter for the horizontal group tab bar. The reorderable widgets
    # are the [All] tab and one tab per group, in layout order; the trailing
    # "+" button is not reorderable and never participates. The filter is
    # installed on every tab button because presses land on the child button
    # and never reach a filter on the parent alone.
    def __init__(self, bar_layout, viewport_widget, parent=None):
        super().__init__(
            viewport_widget,
            get_avatar=None,
            parent=parent,
            row_labels=self._tab_names,
            hold_ms=self.HOLD_TABS_MS,
        )
        self._bar_layout = bar_layout

    def watch(self, widget) -> None:
        """Install this filter on a tab button (and track it for cleanup)."""
        self._targets.add(widget)
        widget.installEventFilter(self)

    def unwatch_all(self) -> None:
        """Remove the filter from previously watched buttons."""
        for widget in list(self._targets):
            if widget is not self._viewport and widget.parent() is not None:
                try:
                    widget.removeEventFilter(self)
                except RuntimeError:
                    pass
        self._targets = {self._viewport}

    def _ordered_widgets(self) -> list:
        widgets = []
        for index in range(self._bar_layout.count()):
            item = self._bar_layout.itemAt(index)
            widget = item.widget() if item else None
            if widget is not None and widget.objectName() == "groupTab" and widget.width() > 24:
                widgets.append(widget)
        return widgets

    def _tab_names(self) -> list[str]:
        return [str(w.text()) for w in self._ordered_widgets()]
...
class _Bridge(QObject):
    ...
    extension_state_changed = Signal(object) # pairing state dict from the extension bridge
    session_started = Signal(object) # OperationResult from a launch profile run
...
class AccountManagerUIQt(QMainWindow): # Main Window
    def __init__(self, manager, icon_path: str | None = None):
        ...
        self._bridge.extension_state_changed.connect(self._on_extension_state_changed)
        self._bridge.session_started.connect(self._on_session_started)
        ...
        # Account list search box and filter chips
        self._account_search: QLineEdit | None = None
        self._filter_invalid = False
        self._filter_running = False
        self._filter_noted = False
        ...
        # Browser extension bridge (pairing code + access token)
        self._extension_bridge = extension_bridge_mod.ExtensionBridge(
            manager=self.manager,
            get_settings=actions.load_ui_settings,
            on_change=lambda state: self._bridge.extension_state_changed.emit(state),
        )
        ...
        if S.get("developer_mode") and (
            S.get("websocket_enabled") or S.get("extension_enabled")
        ):
            self._ws_server.start()
        self._update_extension_controls()
    ...
    def _build_center_panel(self) -> QFrame: # Main account list
        ...
        self._group_bar_widget = _group_bar_widget  # kept for the drag filter
        ...
        _search_row.setSpacing(4)

        self._account_search = QLineEdit()
        self._account_search.setPlaceholderText("Search accounts...")
        self._account_search.setClearButtonEnabled(True)
        self._account_search.setToolTip(
            "Filter by account name or note.\nCtrl+F focuses this box, Esc clears it."
        )
        self._account_search.textChanged.connect(self._on_account_search_changed)
        _search_row.addWidget(self._account_search, 1)

        self._filter_btns: dict[str, QPushButton] = {}
        for _key, _label, _tip in (
            ("invalid", "Invalid", "Show only accounts whose cookie validation failed."),
            ("running", "Running", "Show only accounts with a running Roblox client or an active presence."),
            ("noted", "Noted", "Show only accounts that have a note."),
        ):
            _btn = QPushButton(_label)
            _btn.setObjectName("groupTab")
            _btn.setCheckable(True)
            _btn.setToolTip(_tip)
            _btn.toggled.connect(
                lambda checked, k=_key: self._on_account_filter_toggled(k, checked)
            )
            self._filter_btns[_key] = _btn
            _search_row.addWidget(_btn)

        lay.addLayout(_search_row)

        _search_shortcut = QShortcut(QKeySequence("Ctrl+F"), self)
        _search_shortcut.activated.connect(self._focus_account_search)
        _clear_shortcut = QShortcut(QKeySequence("Escape"), self._account_search)
        _clear_shortcut.setContext(Qt.ShortcutContext.WidgetShortcut)
        _clear_shortcut.activated.connect(self._clear_account_search)
    ...
    def _build_settings_panel(self) -> QFrame: # Settings panel
        ...
        f.addWidget(_sec("SESSIONS"))
        _sess_hint = QLabel(
            "Save a destination, an account set and launch options, then start the\n"
            "whole setup in one click from here or the system tray."
        )
        _sess_hint.setWordWrap(True)
        _sess_hint.setStyleSheet(f"color: {MUTED}; font-size: 10px;")
        f.addWidget(_sess_hint)

        self._session_list = QListWidget()
        self._session_list.setFixedHeight(96)
        self._session_list.itemDoubleClicked.connect(
            lambda _item: self._on_session_start()
        )
        f.addWidget(self._session_list)

        _sess_btn_row = QHBoxLayout()
        _sess_btn_row.setSpacing(6)

        self._sess_start_btn = QPushButton("Start")
        self._sess_start_btn.setToolTip("Launch every account in the selected session.")
        self._sess_start_btn.clicked.connect(self._on_session_start)

        self._sess_stop_btn = QPushButton("Stop")
        self._sess_stop_btn.setToolTip(
            "Stop the Auto-Rejoin workers and Anti-AFK this session started.\n"
            "Roblox clients are left running."
        )
        self._sess_stop_btn.clicked.connect(self._on_session_stop)

        self._sess_new_btn = QPushButton("New")
        self._sess_new_btn.clicked.connect(self._on_session_new)

        self._sess_edit_btn = QPushButton("Edit")
        self._sess_edit_btn.clicked.connect(self._on_session_edit)

        self._sess_delete_btn = QPushButton("Delete")
        self._sess_delete_btn.clicked.connect(self._on_session_delete)

        for _sess_btn in (
            self._sess_start_btn, self._sess_stop_btn, self._sess_new_btn,
            self._sess_edit_btn, self._sess_delete_btn,
        ):
            _sess_btn_row.addWidget(_sess_btn)
        f.addLayout(_sess_btn_row)
        ...
        f.addWidget(_sec("BROWSER EXTENSION"))
        self._sett_ext_chk = _chk(
            "extension_enabled", "Enable Browser Extension",
            "Allow the Evanovar RAM browser extension to connect over the\n"
            "local WebSocket server so you can launch accounts, join friends\n"
            "and save the Roblox account you are logged in as.",
            on_change=self._on_sett_extension_changed,
        )
        f.addWidget(self._sett_ext_chk)

        self._sett_ext_status = QLabel("")
        self._sett_ext_status.setWordWrap(True)
        self._sett_ext_status.setStyleSheet(f"color: {MUTED}; font-size: 10px;")
        f.addLayout(_sub_indent(self._sett_ext_status))

        ext_code_row = QHBoxLayout()
        ext_code_row.setContentsMargins(20, 0, 0, 0)
        self._sett_ext_code_lbl = QLabel("Pairing code")
        self._sett_ext_code_lbl.setToolTip(
            "Enter this code in the extension popup to link it.\n"
            "The code is valid for 5 minutes and can only be used once."
        )
        ext_code_row.addWidget(self._sett_ext_code_lbl)
        ext_code_row.addStretch(1)
        self._sett_ext_code = QLabel("------")
        self._sett_ext_code.setMinimumWidth(96)
        self._sett_ext_code.setAlignment(
            Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter
        )
        self._sett_ext_code.setStyleSheet(
            f"color: {FG_ACCENT}; font-family: Consolas, 'Courier New', monospace;"
            "font-size: 13px; font-weight: 700; letter-spacing: 1px;"
        )
        ext_code_row.addWidget(self._sett_ext_code)
        f.addLayout(ext_code_row)

        ext_btn_row = QHBoxLayout()
        ext_btn_row.setContentsMargins(20, 0, 0, 0)
        self._sett_ext_gen_btn = QPushButton("Generate Pairing Code")
        self._sett_ext_gen_btn.clicked.connect(self._on_sett_extension_generate)
        ext_btn_row.addWidget(self._sett_ext_gen_btn)
        self._sett_ext_copy_btn = QPushButton("Copy")
        self._sett_ext_copy_btn.setFixedWidth(60)
        self._sett_ext_copy_btn.clicked.connect(self._on_sett_extension_copy)
        ext_btn_row.addWidget(self._sett_ext_copy_btn)
        self._sett_ext_unlink_btn = QPushButton("Unlink")
        self._sett_ext_unlink_btn.setFixedWidth(60)
        self._sett_ext_unlink_btn.clicked.connect(self._on_sett_extension_unlink)
        ext_btn_row.addWidget(self._sett_ext_unlink_btn)
        f.addLayout(ext_btn_row)

        self._sett_ext_hint = QLabel(
            "Load the browser-extension folder as an unpacked extension, open its\n"
            "popup, then enter the pairing code to link it."
        )
        self._sett_ext_hint.setWordWrap(True)
        self._sett_ext_hint.setStyleSheet(f"color: {MUTED}; font-size: 10px;")
        f.addLayout(_sub_indent(self._sett_ext_hint))

        self._ext_pair_code = ""
        self._ext_pair_timer = QTimer(self)
        self._ext_pair_timer.setInterval(1000)
        self._ext_pair_timer.timeout.connect(self._update_extension_controls)
        self._update_extension_controls()
    ...
    def _on_sett_developer_mode(self, enabled: bool):
        ...
        if hasattr(self, "_sett_ext_chk"):
            self._sett_ext_chk.setEnabled(enabled)
        self._update_ws_password_controls()
        if not enabled:
            ...
            if hasattr(self, "_sett_ext_chk") and self._sett_ext_chk.isChecked():
                self._sett_ext_chk.setChecked(False)
            self._extension_bridge.cancel_pairing()
        self._update_extension_controls()
    ...
    def _on_sett_extension_changed(self, enabled: bool):
        if enabled:
            # The extension talks to the app over the WebSocket server.
            if hasattr(self, "_sett_ws_chk") and not self._sett_ws_chk.isChecked():
                self._sett_ws_chk.setChecked(True)
            else:
                self._start_ws_server()
        else:
            self._extension_bridge.cancel_pairing()
    ...
    def _on_sett_extension_generate(self):
        result = self._extension_bridge.begin_pairing()
        if not result:
            self._show_operation_error(result)
        self._update_extension_controls()

    def _on_sett_extension_copy(self):
        code = str(getattr(self, "_ext_pair_code", "") or "")
        if not code:
            return
        QApplication.clipboard().setText(code)
        print("[INFO] Pairing code copied to the clipboard")

    def _on_sett_extension_unlink(self):
        if not self._extension_bridge.is_paired():
            return
        reply = QMessageBox.question(
            self,
            "Unlink Browser Extension",
            "Unlink the browser extension?\n\n"
            "The extension will need a new pairing code to reconnect.",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
        )
        if reply != QMessageBox.StandardButton.Yes:
            return
    ...
    def _on_extension_state_changed(self, state):
        # Queued from the WebSocket thread when the extension pairs or unlinks.
        self._update_extension_controls()

    def _update_extension_controls(self):
        if not hasattr(self, "_sett_ext_status"):
            return

        developer_mode = bool(
            hasattr(self, "_sett_devmode_chk") and self._sett_devmode_chk.isChecked()
        )
        enabled = bool(
            hasattr(self, "_sett_ext_chk") and self._sett_ext_chk.isChecked()
        )
        websocket_on = bool(
            hasattr(self, "_sett_ws_chk") and self._sett_ws_chk.isChecked()
        )

        state = self._extension_bridge.pairing_state()
        pairing_active = bool(state.get("pairing_active"))
        paired = bool(state.get("paired"))
        code = str(state.get("pairing_code") or "")

        self._ext_pair_code = code
        self._sett_ext_code.setText(code or "------")

        self._sett_ext_gen_btn.setEnabled(developer_mode and enabled)
        self._sett_ext_copy_btn.setEnabled(developer_mode and enabled and bool(code))
        self._sett_ext_unlink_btn.setEnabled(developer_mode and paired)
        self._sett_ext_code_lbl.setEnabled(developer_mode and enabled)

        if not developer_mode:
            status = "Developer Mode is off."
        elif not enabled:
            status = "Disabled. Turn this on to link the extension."
        elif not websocket_on:
            status = "The WebSocket server is off, so the extension cannot connect."
        elif pairing_active:
            remaining = int(state.get("seconds_remaining", 0) or 0)
            status = f"Waiting for the extension - code expires in {remaining}s"
        elif paired:
            status = "Linked. The extension can launch accounts and join friends."
        else:
            status = "Not linked. Generate a pairing code and enter it in the extension."
        self._sett_ext_status.setText(status)

        if pairing_active:
            if not self._ext_pair_timer.isActive():
                self._ext_pair_timer.start()
        else:
            self._ext_pair_timer.stop()

    # ---- Launch profiles (sessions) -------------------------------------

    def _session_selected_name(self) -> str:
    ...
    def _select_session_in_list(self, name: str) -> None:
    ...
    def _refresh_session_list(self, select: str = "") -> None:
        if not hasattr(self, "_session_list"):
            return

        wanted = select or self._session_selected_name()
        self._session_list.clear()

        for session in sessions_mod.load_sessions():
            name = str(session.get("name") or "")
            if not name:
                continue

            place_id = str(session.get("place_id") or "")
            destination = f"place {place_id}" if place_id else "private server"
            if session.get("job_id"):
                destination += " + job"

            configured = [str(a) for a in (session.get("accounts") or [])]
            count = len(configured)
            missing = len([a for a in configured if a not in self.manager.accounts])
            ...
            summary = f"{name}  -  {destination}, {count} account{'s' if count != 1 else ''}"
            if flags:
            ...
            item.setToolTip(
                ...
                f"Delay: {session.get('delay_seconds', 0)}s\n"
                f"Accounts: {', '.join(configured) if configured else 'none'}"
            )
            self._session_list.addItem(item)

        has_sessions = self._session_list.count() > 0
        for button in (
            self._sess_start_btn, self._sess_stop_btn,
            self._sess_edit_btn, self._sess_delete_btn,
        ):
            button.setEnabled(has_sessions)

        if has_sessions:
            self._select_session_in_list(wanted)

    def _on_session_new(self):
        dialog = _SessionEditDialog(sorted(self.manager.accounts.keys()), self)
        if dialog.exec() != QDialog.DialogCode.Accepted or not dialog.result_session:
            return
        if not sessions_mod.save_session(dialog.result_session):
            self._show_operation_error(OperationResult.failure(
                "SESSION_SAVE_FAILED",
            ...
            return
        print(f"[INFO] Saved session '{dialog.result_session['name']}'")
        self._refresh_session_list(select=dialog.result_session["name"])

    def _on_session_edit(self):
        name = self._session_selected_name()
        if not name:
            return
        session = sessions_mod.get_session(name)
        if not session:
            return
        dialog = _SessionEditDialog(sorted(self.manager.accounts.keys()), self, session)
        if dialog.exec() != QDialog.DialogCode.Accepted or not dialog.result_session:
            return
        if not sessions_mod.save_session(dialog.result_session):
            self._show_operation_error(OperationResult.failure(
            ...
            return
        print(f"[INFO] Updated session '{dialog.result_session['name']}'")
        self._refresh_session_list(select=dialog.result_session["name"])
    ...
    def _on_session_delete(self):
        ...
        reply = QMessageBox.question(
            self,
            "Delete Session",
            f"Delete the session '{name}'?\n\nThis does not affect saved accounts.",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
        )
        if reply != QMessageBox.StandardButton.Yes:
            return
        if sessions_mod.delete_session(name):
            print(f"[INFO] Deleted session '{name}'")
        self._refresh_session_list()
    ...
    def _on_session_start(self):
        ...
        if not name:
            return
        self._start_session_by_name(name)
    ...
    def _on_session_stop(self):
        ...
        accounts = [str(a) for a in (session.get("accounts") or [])]
        stopped = ar.disarm_accounts(accounts, self._ar_workers)
        if session.get("anti_afk"):
            actions.stop_anti_afk()
        self._refresh_account_list()
        print(f"[INFO] Session '{name}': stopped {len(stopped)} Auto-Rejoin worker(s)")

    def _start_session_by_name(self, name: str) -> bool:
        session = sessions_mod.get_session(name)
        if not session:
            self._show_operation_error(OperationResult.failure(
                "SESSION_NOT_FOUND",
                "Session Not Found",
                f"The session '{name}' no longer exists.",
            ))
            return False
        ...
        if not accounts:
            self._show_operation_error(OperationResult.failure(
                ...
                "None of this session's accounts are still saved.",
            ))
            return False

        if session.get("auto_rejoin"):
            _, skipped = ar.arm_accounts(
                self.manager,
                accounts,
                self._ar_configs,
                self._ar_workers,
                on_status=lambda user, status: self._bridge.rejoin_status.emit(user, status),
            )
            if skipped:
                print(
                    f"[WARNING] Session '{name}': no Auto-Rejoin configuration for "
                    f"{', '.join(skipped)}"
                )
        ...
        actions.start_session(self.manager, target, on_done=self._on_session_start_done)

        print(f"[INFO] Starting session '{name}' with {len(accounts)} account(s)")
        if missing:
            print(
                f"[WARNING] Session '{name}' skipped unsaved accounts: {', '.join(missing)}"
            )
        self._refresh_account_list()
        return True

    def _on_session_start_done(self, ok, result):
        # Runs on the session worker thread; hop back via the bridge signal.
        self._bridge.session_started.emit(result)

    def _on_session_started(self, result):
        operation_result = ensure_result(
            result,
            failure_code="SESSION_START_FAILED",
            failure_title="Session Could Not Start",
            failure_message="The session could not be started.",
        )
        if not operation_result:
            self._show_operation_error(operation_result)
        elif operation_result.message:
            print(f"[SUCCESS] {operation_result.message}")
        self._refresh_account_list()
    ...
    def _rebuild_tray_sessions_menu(self) -> None:
        """Refresh the tray session list each time the menu opens."""
        menu = getattr(self, "_tray_sessions_menu", None)
        if menu is None:
            return
        ...
        for session in sessions:
            ...
            action.triggered.connect(
                lambda _checked=False, n=name: self._start_session_by_name(n)
            )
    ...
    def _enable_system_tray(self) -> OperationResult:
        ...
        try:
            ...
            menu.addSeparator()
            self._tray_sessions_menu = menu.addMenu("Sessions")
            menu.aboutToShow.connect(self._rebuild_tray_sessions_menu)
            menu.addSeparator()
    ...
    def _perform_shutdown_cleanup(self) -> None:
        ...
        # Stop extension pairing timer
        try:
            if hasattr(self, "_ext_pair_timer"):
                self._ext_pair_timer.stop()
        except Exception:
            pass
        ...
        try:
            ...
            pass
        try:
            if hasattr(self, "_group_drag_filter"):
                self._group_drag_filter.cleanup()
        except Exception:
            pass
    ...
    def _refresh_account_list(self): # Main account list population and refresh
        ...
        # Filter by the search box and the filter chips
        account_items = self._apply_account_filters(account_items)

        if not account_items:
            if self.manager.accounts and self._account_filters_active():
                empty_text = "No accounts match the current search or filters."
            else:
                empty_text = "No accounts, use 'Add Account' to add one."
    ...
    def _on_account_search_changed(self, _text: str):
        self._refresh_account_list()

    def _on_account_filter_toggled(self, key: str, checked: bool):
        if key == "invalid":
            self._filter_invalid = bool(checked)
        elif key == "running":
            self._filter_running = bool(checked)
        elif key == "noted":
            self._filter_noted = bool(checked)
        self._refresh_account_list()

    def _focus_account_search(self):
        if self._account_search is not None:
            self._account_search.setFocus()
            self._account_search.selectAll()

    def _clear_account_search(self):
        if self._account_search is not None and self._account_search.text():
            self._account_search.clear()

    def _account_filters_active(self) -> bool:
        query = ""
        if self._account_search is not None:
            query = self._account_search.text().strip()
        return bool(
            query or self._filter_invalid or self._filter_running or self._filter_noted
        )

    def _running_accounts(self) -> set:
        """Accounts with a running client or a live presence reading."""
        running = set(getattr(self, "_online_usernames", None) or set())
        snapshot = getattr(self, "_activity_snapshot", None)
        if snapshot:
            running.update(snapshot.keys())
        return running

    def _apply_account_filters(self, account_items: list) -> list:
        items = list(account_items)

        if self._account_search is not None:
            query = self._account_search.text().strip().casefold()
            if query:
                def _matches(username: str, data) -> bool:
                    note = ""
                    if isinstance(data, dict):
                        note = str(data.get("note", "") or "")
                    return query in str(username).casefold() or query in note.casefold()

                items = [(u, d) for u, d in items if _matches(u, d)]

        if self._filter_invalid:
            items = [
                (u, d) for u, d in items
                if isinstance(d, dict) and self._cv_mod.is_flagged(d)
            ]

        if self._filter_noted:
            items = [
                (u, d) for u, d in items
        ...
        if self._filter_running:
            running = self._running_accounts()
            items = [(u, d) for u, d in items if u in running]
    ...
    def _rebuild_group_bar(self):
        ...
        # The bar's buttons are recreated here, so the drag filter must be
        # re-attached to each new button after the rebuild.
        if getattr(self, "_group_drag_filter", None) is not None:
            self._group_drag_filter.unwatch_all()
        ...
        all_btn.setToolTip("All accounts. Drag tabs to reorder groups.")
        ...
        # Swipe tabs left/right to reorder; the order is saved automatically.
        if getattr(self, "_group_drag_filter", None) is None:
            self._group_drag_filter = _GroupTabDragFilter(
                self._group_bar_lay,
                self._group_bar_widget,
                parent=self,
            )
            self._group_drag_filter.reorder_requested.connect(
                self._on_group_reorder
            )
        # Every new button needs the filter; without it, presses on a tab
        # never reach the drag logic (the bar itself only sees presses on its
        # own padding).
        for widget in self._group_drag_filter._ordered_widgets():
            self._group_drag_filter.watch(widget)

    def _on_group_reorder(self, from_row: int, insert_before_row: int):
        # "from_row" indexes the reorderable tabs: 0 is [All], 1..N are groups.
        # The persisted group order starts at 0, so shift both by one. The
        # drop index means "insert before tab T of the bar as it was before
        # the move", so a target past the old position shifts left by one —
        # same rule the account list uses.
        old_index = from_row - 1
        target_index = insert_before_row - 1
        if old_index < 0:
            return
        if target_index > old_index:
            target_index -= 1
        try:
            changed = groups.move_group(old_index, target_index)
        except Exception as e:
            print(f"[WARNING] Could not move group: {e}")
            changed = False
        if changed:
            print(f"[INFO] Moved group '{groups.get_group_names()[target_index]}' to position {target_index + 1}.")
        self._rebuild_group_bar()
    ...
    def _on_account_reorder(self, from_row: int, insert_before_row: int): # Reoder accounts
        ...
        # The drop index means "insert before row T of the list as it was
        # before the move". Once the item is popped, a target past the old
        # position must shift left by one so "before T" keeps referring to
        # the same neighbour (e.g. dropping row 0 before row 2 lands it at
        # index 1, right where it was visually dropped).
...
class _SessionEditDialog(QDialog):
    """Create or edit a saved launch profile."""

    def __init__(self, accounts: list[str], parent, session: dict | None = None):
        super().__init__(parent)
        self._existing = dict(session or {})
        self._accounts = sorted(str(name) for name in accounts)
        self.result_session: dict | None = None

        is_new = session is None
        title = "New Session" if is_new else f"Edit Session - {self._existing.get('name', '')}"
        ...
        self._name_edit = _field(
            "Name", QLineEdit(str(self._existing.get("name", "")))
        )
        self._name_edit.setPlaceholderText("Evening farm")

        self._place_edit = _field(
            "Place ID", QLineEdit(str(self._existing.get("place_id", "")))
        )
        self._place_edit.setPlaceholderText("123456789")

        self._private_edit = _field(
            "Private server link (optional)",
            QLineEdit(str(self._existing.get("private_server", ""))),
        )
        self._private_edit.setPlaceholderText("https://www.roblox.com/games/...")

        self._job_edit = _field(
            "Job ID (optional)", QLineEdit(str(self._existing.get("job_id", "")))
        )
        self._job_edit.setPlaceholderText("Specific server to join")

        self._delay_spin = _field("Delay between launches", QDoubleSpinBox())
        self._delay_spin.setRange(0.0, 300.0)
        self._delay_spin.setDecimals(1)
        self._delay_spin.setSingleStep(0.5)
        self._delay_spin.setSuffix(" s")
        try:
            self._delay_spin.setValue(
                float(self._existing.get("delay_seconds", sessions_mod.DEFAULT_DELAY_SECONDS))
            )
        except (TypeError, ValueError):
            self._delay_spin.setValue(sessions_mod.DEFAULT_DELAY_SECONDS)

        # Accounts
        accounts_caption = QLabel("Accounts")
        accounts_caption.setStyleSheet(f"color: {MUTED}; font-size: 10px;")
        lay.addWidget(accounts_caption)

        self._account_list = QListWidget()
        self._account_list.setFixedHeight(120)
        selected = {str(name) for name in (self._existing.get("accounts") or [])}
        ...
        self._ar_chk = QCheckBox("Arm Auto-Rejoin for these accounts")
        self._ar_chk.setChecked(bool(self._existing.get("auto_rejoin", False)))
        self._ar_chk.setToolTip(
            "Auto-Rejoin only starts for accounts that already have an Auto-Rejoin configuration."
        )
        lay.addWidget(self._ar_chk)

        self._afk_chk = QCheckBox("Start Anti-AFK")
        self._afk_chk.setChecked(bool(self._existing.get("anti_afk", False)))
        self._afk_chk.setToolTip("Uses the Anti-AFK key and interval saved on the main window.")
        lay.addWidget(self._afk_chk)
    ...
    def _set_all_checked(self, checked: bool) -> None:
        ...
        for index in range(self._account_list.count()):
            self._account_list.item(index).setCheckState(state)

    def _checked_accounts(self) -> list[str]:
        return [
            self._account_list.item(index).text()
            for index in range(self._account_list.count())
            if self._account_list.item(index).checkState() == Qt.CheckState.Checked
        ]

    def _accept(self) -> None:
        name = self._name_edit.text().strip()
        if not name:
            QMessageBox.warning(self, "Name Required", "Give this session a name.")
            return

        place_id = self._place_edit.text().strip()
        private_server = self._private_edit.text().strip()
        if place_id and not place_id.isdigit():
            QMessageBox.warning(self, "Invalid Place ID", "The Place ID must be numeric.")
            return
        if not place_id and not private_server:
            QMessageBox.warning(
                self,
                "Destination Required",
                "Set a Place ID or a private server link.",
            )
            return

        accounts = self._checked_accounts()
        if not accounts:
            QMessageBox.warning(self, "Accounts Required", "Select at least one account.")
            return
        ...
        self.result_session = {
            ...
            "job_id": self._job_edit.text().strip(),
            "accounts": accounts,
            "delay_seconds": float(self._delay_spin.value()),
            "auto_rejoin": self._ar_chk.isChecked(),
            "anti_afk": self._afk_chk.isChecked(),
            "created_at": self._existing.get("created_at"),
        }
        self.accept()
